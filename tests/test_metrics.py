...
@pytest.mark.parametrize("metric_cls", [DifficultyMetric, ClarityMetric, DistractorQualityMetric])
...
def test_distractor_quality_phase_requires_question():
    """Distractor quality prompt builder should raise ValueError when question is missing."""
    metric = DistractorQualityMetric()
    inp = make_phase_input(metric, "score")
    with pytest.raises(ValueError, match="requires a question"):
        inp.prompt_builder(inp)

def test_distractor_quality_phase_builds_prompt():
    """Distractor quality prompt builder should return a non-empty string."""
    metric = DistractorQualityMetric()
    inp = make_phase_input(metric, "score", question=make_question())
    prompt = inp.prompt_builder(inp)
    assert isinstance(prompt, str)
